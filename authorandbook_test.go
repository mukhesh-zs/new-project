...
func TestGetbyId(t *testing.T) {
	testcase := []struct {
		...
		{"valid id", "1", http.StatusOK, book{1, "2states",
			&author{1, "chetan", "bhagat", "1980/04/23", "max"},
			"arihant", "2020/10/23"}},
...
func TestGetAll(t *testing.T) {
	testcases := []struct {
		...
		{"getting data of all books", "/books",
			[]book{{1, "2states", &author{1, "chetan", "bhagat", "1980/04/23", "max"},
				"arihant", "2020/10/23"},
				{2, "3states", &author{2, "mukhesh", "chandra", "2000/10/24", "rolex"},
					"penguin", "2000/07/03"}},
		},
	}
...
func TestPostBook(t *testing.T) {
	testcases := []struct {
		...
		{"details posted for id 2", "/books", book{2, "god", &author{1, "mukhesh", "mekala", "2000/10/24", "zero"}, "arihant", "2015/07/03"}, http.StatusCreated},
		{"invalid input title", "/books", book{2, "", &author{1, "mukhesh", "mekala", "2000/10/24", "zero"}, "arihant", "2015/07/03"}, http.StatusBadRequest},
	}
	for _, tc := range testcases {
...
func TestPostAuthor(t *testing.T) {
	testcases := []struct {
		...
		{"valid author", author{1, "chetan", "bhagat", "1980/04/23", "max"}, http.StatusOK},
		{"author name not given", author{2, "", "kumar", "1906/06/16", "rolex"}, http.StatusBadRequest},
	}
	for _, tc := range testcases {
		data, _ := json.Marshal(tc.input)
...
func TestPutAuthor(t *testing.T) {
	testcases := []struct {
		...
		{"updating details", "1", author{1, "chetan", "bhagat", "1980/04/23", "rolex"}, http.StatusOK},
	}
	for _, tc := range testcases {
		body, _ := json.Marshal(tc.input)
...
func TestDeleteBook(t *testing.T) {
	testcases := []struct {
		...
		{"valid id", "3", http.StatusNoContent},
